...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        npy_pth = npz_dir + os.path.splitext(filename)[0] + '.npz'
    ...
    def read_video(self, video_filename, filename, width=224, height=224):
        ...
        try:
            ...
            npy_pth = npz_dir + os.path.splitext(filename)[0]
    ...
    def adjust_frames(self, frames):
        ...
        if self.num_frames <= len(frames):
            for i in range(self.num_frames):
                frame = frames[i * len(frames) // self.num_frames] # // 整除的意思
...
if __name__ == '__main__':
    ...
    for split in range(len(tag)):
        npz_dir = r'/data0/wanghang/VRAC_2/dataset_REPCountA_resume/video_npz_64_raw/'
        mod = tag[split]
