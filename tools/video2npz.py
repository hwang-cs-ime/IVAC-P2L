    ...
    def read_video(self, video_filename, filename, width=224, height=224):
        ...
        try:
            ...
            if (frames.size != 0):
                ...
                return -1
            # npz_pth = r'/p300/LSP_npz/npz_data/' + os.path.splitext(filename)[0]
            npz_pth = npz_dir + os.path.splitext(filename)[0]
            np.savez(npz_pth, imgs=frames, fps=original_frames_length)  # [f,c,h,w]
        except:
...
if __name__ == '__main__':
    ...
    for split in range(len(tag)):
        ...
        test = MyDataset(data_root, label_file, num_video_Norm, mod)
        npz_dir = npz_dir + mod + '/'
        print('=========================================')
        print(mod, ' : ', npz_dir)
